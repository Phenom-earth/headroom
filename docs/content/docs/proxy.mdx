...
Telemetry is hard-disabled in this Phenom-earth fork: the anonymous beacon and the usage phone-home are off regardless of `HEADROOM_TELEMETRY` or `--no-telemetry`.
